...
try:
    from .mdf import mdf_skeleton, dataField, conversionField 
...
class record(list):
    ...
    def readSortedRecord(self, fid, pointer, channelList=None, loadFast=True, noDataLoading=False):
        ...
        """
        fid.seek(pointer)
        if (channelList is None or loadFast) and (not noDataLoading):  # reads all, quickest but memory consuming
           return fromfile(fid, dtype=self.numpyDataRecordFormat, shape=self.numberOfRecords, names=self.dataRecordName)
            ...
            if len(list(set(channelList))+ list(set(self.channelNames))) > 0:  # are channelList in this dataGroup
                ...
                if not noDataLoading:
                    ...
                    rec = zeros((1, ), dtype=numpyDataRecordFormat)
                    recordLength = 0
...
class DATA(dict):
    ...
    def read(self, channelList, zip=None, noDataLoading=False):
        """Reads data block
        ...
        if len(self) == 1:  # sorted dataGroup
            ...
            self[recordID]['data'] = self.loadSorted(self[recordID]['record'], zip=None, nameList=channelList, noDataLoading=noDataLoading)
        else:  # unsorted DataGroup
            self.load(nameList=channelList)


    def loadSorted(self, record, zip=None, nameList=None, noDataLoading=False):  # reads sorted data
        """Reads sorted data block from record definition
        ...
        """
        return record.readSortedRecord(self.fid, self.pointerToData, nameList, noDataLoading=noDataLoading)
...
class mdf3(mdf_skeleton):
    ...
    def read3(self, fileName=None, info=None, multiProc=False, channelFilter=['.*'], channelList=None, convertAfterRead=True, filterChannelNames=False, noDataLoading=False):
        """ Reads mdf 3.x file data and stores it in dict
        ...
        if channelFilter != ['.*']:
            channelList = info.listChannels(None, channelFilter)
        ...
        for dataGroup in info['DGBlock'].keys():
            if info['DGBlock'][dataGroup]['numberOfChannelGroups'] > 0:  # data exists
                ...
                buf.read(channelList, noDataLoading=noDataLoading) # reads datablock potentially containing several channel groups
                
                for recordID in buf.keys():
                    if 'record' in buf[recordID]:
                        master_channel = buf[recordID]['record'].master['name']
                        
                        if master_channel in self.keys():# or master_channel[:4] == 'time':
                            master_channel += '_' + str(dataGroup)
                          
                        for chan in buf[recordID]['record']:
                            if not allChannel and master_channel not in channelList:
                                channelList.append(master_channel)  # adds master channel
                        ...
                        for chan in buf[recordID]['record']: # for each recordchannel
                            ...
                            if (allChannel or chan.name in channelList):
                                if not noDataLoading:
                                    try:
                                        temp = buf[recordID]['data'].__getattribute__(str(recordName) + '_title')
                                    except:
                                        temp = buf[recordID]['data'].__getattribute__(str(chan.name) + '_title')
                                else:
                                    temp = None
                                
                                
                                # Process concatenated bits inside uint8
                                if (not chan.bitCount // 8.0 == chan.bitCount / 8.0) and not noDataLoading:  # if channel data do not use complete bytes
        ...
        if convertAfterRead and not noDataLoading:
