...
class mdfinfo(dict):
    ...
    def listChannels(self, fileName=None, channelFilter=['.*']):
        ...
        if self.mdfversion < 400:  # up to version 3.x not compatible with version 4.x
            ...
            nameList = channelNameList.listChannels3(self.fileName, channelFilter)
        else:
            from .mdfinfo4 import info4
            channelNameList = info4()
            nameList = channelNameList.listChannels4(self.fileName, channelFilter)
        return nameList
...
class mdf(mdf3, mdf4):
    ...
    def read(self, fileName=None, multiProc=False, channelFilter=['.*'], channelList=None, convertAfterRead=True, filterChannelNames=False, noDataLoading=False):
        """ reads mdf file version 3.x and 4.x
        ...
        if self.MDFVersionNumber < 400:  # up to version 3.x not compatible with version 4.x
            self.read3(self.fileName, info, multiProc, channelFilter, channelList, convertAfterRead, filterChannelNames=filterChannelNames, noDataLoading=noDataLoading)
        else:  # MDF version 4.x. Channel by channel reading implemented
            self.read4(self.fileName, info, multiProc, channelFilter, channelList, convertAfterRead, filterChannelNames=filterChannelNames)
