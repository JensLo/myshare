...
path.append(_root)
from mdf import _open_MDF, dataField, descriptionField, unitField, masterField, masterTypeField, deviceField
...
class info4(dict):
    ...
    def __init__(self, fileName=None, fid=None, channelFilter=['.*']):
    ...
    def listChannels4(self, fileName=None, fid=None, channelFilter=['.*']):
        ...
        for dg in self['DGBlock']:
            for cg in self['CGBlock'][dg]:
                for cn in self['CNBlock'][dg][cg]:
                    signalName = self['CNBlock'][dg][cg][cn]['name'] 
                    for cf in atleast_1d( channelFilter ):
                        if ( re.findall(cf, signalName) != [] ):
...
def _generateDummyMDF4(info, channelFilter, channelList):
