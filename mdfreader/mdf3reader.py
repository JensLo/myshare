...
from mdf import mdf_skeleton, _open_MDF, _bits_to_bytes, _convertName, dataField, deviceField, conversionField, compressed_data
...
class record(list):
    ...
    def addChannel(self, info, channelNumber):
        ...
        self.channelNames.add(_convertName(self[-1].name))
    ...
    def loadInfo(self, info):
        ...
        for channelNumber in list(info['CNBlock'][self.dataGroup][self.channelGroup].keys()):
            ...
            self.append(channel)
            self.channelNames.add(_convertName(channel.name))
            ...
            if len(self) > 1:
                ...
                if channel.posBitBeg >= 8 * self[-2].byteOffset \
                    ...
                    if self.recordToChannelMatching: # not first channel
                        ...
                        print(channel.recAttributeName)
    ...
    def readSortedRecord(self, fid, pointer, channelSet=None, loadFast=True):
        ...
        """
        fid.seek(pointer)
        if channelSet is None and not self.hiddenBytes and self.byte_aligned and loadFast:  # reads all, quickest but memory consuming
            ...
            if len(channelSet & self.channelNames) > 0:  # are channelSet in this dataGroup
                ...
                """try:  # use rather cython compiled code for performance
                    ...
                    for chan in range(len(self)):
                        if self[chan].recAttributeName in channelSet:
                            buf[self[chan].recAttributeName] = dataRead.dataRead(bytes(bita), self[chan].bitCount, \
                ...
                for channel in channelSet:  # initialise data structure
                    rec[channel] = 0
                    if channel in self.recordToChannelMatching:
                        if self.recordToChannelMatching[channel] not in channelSet:
                            channelsToAdd.append(self.recordToChannelMatching[channel])
                channelSet.update(channelsToAdd)
...
class DATA(dict):
    ...
    def read(self, channelSet):
        ...
        if channelSet is not None:
            channelSet = set([_convertName(name) for name in channelSet] )
...
class mdf3(mdf_skeleton):
    ...
    def read3(self, fileName=None, info=None, multiProc=False, channelFilter=['.*'], channelList=None, \
        ...
        if channelFilter != ['.*'] and channelList is None:
    ...
    def _getChannelData3(self, channelName):
        ...
        if channelName in self:
            chan = self.getChannel(channelName)
            vect = chan[dataField]
            if vect is None: # noDataLoading reading argument flag activated
                ...
                channelList = [channelName, chan['master']]
                self.read3(fileName=None, info=self._info, channelList=channelList, convertAfterRead=False)
            return self._convert3(channelName)
        else:
            raise KeyError('Channel ' + channelName + ' not in mdf dictionary')
            return channelName

    def _getAllChannelData3(self):
        if self._info.fid is None or (self._info.fid is not None and self._info.fid.closed):
            (self._info.fid, self._info.fileName, self._info.zipfile) = _open_MDF(self.fileName)
        self.read3(fileName=None, info=self._info, channelList=None, convertAfterRead=False)
