...
from numpy import sort, zeros, atleast_1d
from struct import calcsize, unpack
from mdf import dataField, descriptionField, unitField, masterField, masterTypeField, deviceField
import re
...
class info3(dict):
        ...
        for dataGroup in range(self['HDBlock']['numberOfDataGroups']):
            ...
            for channelGroup in range(self['DGBlock'][dataGroup]['numberOfChannelGroups']):
                ...
                for channel in range(self['CGBlock'][dataGroup][channelGroup]['numberOfChannels']):
                    ...
                    if self['CNBlock'][dataGroup][channelGroup][channel]['channelType']:
                        signalname += '_' + str(dataGroup)

                    #signalname = signalname.split('\\')
                    if len(signalname) > 1:
                        self['CNBlock'][dataGroup][channelGroup][channel]['deviceName'] = signalname.split('\\')[-1]
                    #signalname = signalname[0]
                    if self.filterChannelNames:
                        signalname = signalname.split('.')[0]  # filters channels modules

                    if signalname in snames:
                        self['CNBlock'][dataGroup][channelGroup][channel]['signalName'] = signalname + '_' + str(channel)
                        self['CNBlock'][dataGroup][channelGroup][channel]['shortSignalName'] = signalname.split('\\')[0] + '_' + str(channel)
                        ...
                        self['CNBlock'][dataGroup][channelGroup][channel]['shortSignalName'] = signalname.split('\\')[0]
    ...
    def listChannels3(self, fileName=None, fid=None, channelFilter=['.*']):
        ...
        for dataGroup in range(self['HDBlock']['numberOfDataGroups']):
            ...
            for channelGroup in range(self['DGBlock'][dataGroup]['numberOfChannelGroups']):
                ...
                for channel in range(self['CGBlock'][dataGroup][channelGroup]['numberOfChannels']):
                    ...
                    # Filter channel by regular expression
                    addChannel = False
                    for cf in atleast_1d( channelFilter ):
                        if ( re.findall(cf, shortSignalName) != [] ):
                            addChannel = True
...
def _generateDummyMDF3(info, channelFilter, channelList):
            ...
            for cg in info['CGBlock'][dg]:
                ...
                for cn in info['CNBlock'][dg][cg]:
                    ...
                    if channelList is None or name in channelList:
                        ...
                        mdfdict[name][deviceField] = info['CNBlock'][dg][cg][cn]['deviceName']
