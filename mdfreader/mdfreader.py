...
class mdfinfo(dict):
    ...
    def listChannels(self, fileName=None, channelFilter=['.*']):
        ...
        if self.mdfversion < 400:  # up to version 3.x not compatible with version 4.x
            ...
            nameList = channelNameList.listChannels3(self.fileName, self.fid, channelFilter)
        else:
            from mdfinfo4 import info4
            channelNameList = info4()
            #nameList = channelNameList.listChannels4(self.fileName, self.fid)
            nameList = channelNameList.listChannels4(self.fileName, self.fid, channelFilter)
            if zipfile: # not from mdfreader.read()
                remove(self.fileName)
        return nameList

    def _generateDummyMDF(self, channelFilter=None, channelList=None):
        ...
        if self.mdfversion < 400:  # up to version 3.x not compatible with version 4.x
            ...
            return _generateDummyMDF3(self, channelFilter, channelList)
        else:  # MDF version 4.x
            from mdfinfo4 import _generateDummyMDF4
            return _generateDummyMDF4(self, channelFilter, channelList)
...
class mdf(mdf3, mdf4):
    ...
    def read(self, fileName=None, multiProc=False, channelFilter=['.*'], channelList=None, \
        ...
        # read file blocks
        self._info = mdfinfo(self.fileName, filterChannelNames)
        self.MDFVersionNumber = self._info.mdfversion

        if not noDataLoading:
            if self.MDFVersionNumber < 400:  # up to version 3.x not compatible with version 4.x
                self.read3(self.fileName, self._info, multiProc, channelFilter, channelList, \
                    convertAfterRead, filterChannelNames, compression)
            else:  # MDF version 4.x
                self.read4(self.fileName, self._info, multiProc, channelFilter, channelList, \
                    convertAfterRead, filterChannelNames, compression)
        else: # populate minimum mdf structure
            (self.masterChannelList, mdfdict) = self._info._generateDummyMDF(channelFilter, channelList)
            self.update(mdfdict)
        ...
        **or**
        ...
        if self: # mdf contains data 
            ...
            if len(list(self.masterChannelList.keys())) > 1:  # Not yet resampled or only 1 datagroup
                    ...
                    try:
                        if Name not in list(self.masterChannelList.keys()):  # not a master channel
                            ...
                            if not self.getChannelData(Name).dtype.kind in ('S', 'U'):  # if channel not array of string
                                f = interp1d( timevect, self.getChannelData(Name), kind = kind, bounds_error=False )
                                self.setChannelData(Name, f( masterData ))
                ...
                for Name in list(self.keys()):
                    timevect = self.getChannelData(self.getChannelMaster(Name))
                    f = interp1d( timevect, self.getChannelData(Name), kind ='nearest', bounds_error=False )
                    self.setChannelData(Name, f( masterData ))
                    self.setChannelMaster(Name, masterChannelName)
                    self.setChannelMasterType(Name, self.getChannelMasterType(master))
                    self.remove_channel_conversion(Name)
    ...
    def convertToPandas(self, sampling=None):
        ...
        if self._noDataLoading:
            if self.MDFVersionNumber < 400:  # up to version 3.x not compatible with version 4.x
                self._getAllChannelData3()
            else:  # MDF version 4.x
                self._getAllChannelData4()
